...
public class CRC {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < frameSize; i++) frame[i] = sc.nextInt();
        ...
        for (int i = 0; i < frameSize; i++) transmitted[i] = frame[i];
        ...
        System.out.println();
        for (int i = 0; i < gp; i++) transmitted[frameSize + i] = remainder[i];
        ...
        for (int i = 0; i < rsize; i++) received[i] = sc.nextInt();
        ...
        if (error) System.out.println("Error Detected");
    ...
    private static int[] divide(int[] dividend, int[] divisor) {
        ...
        for (int i = 0; i <= dividend.length - divisor.length; i++) {
            if (dividend[i] == 1) {
                for (int j = 0; j < divisor.length; j++) dividend[i + j] ^= divisor[j];
            }
        }
        int[] remainder = new int[gp];
        for (int i = 0; i < gp; i++) remainder[i] = dividend[dividend.length - gp + i];
        return remainder;
    }
}
